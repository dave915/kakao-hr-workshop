...
### 공지는 게시됐는데 푸시가 보이지 않을 때

1. 받는 기기에서 앱을 새로고침하고 업데이트 안내가 나오면 적용합니다.
2. `나의 탐험 여권 → 워크샵 소식 받기`에서 알림을 켭니다. 이미 연결되어 있으면 `알림 다시 연결하기`로 현재 기기를 서버에 재등록할 수 있습니다. 연결이 끊겼다는 안내가 나오면 해당 항목을 눌러 다시 연결합니다.
3. 기기 설정에서 앱 또는 사용 중인 브라우저의 알림과 해당 사이트의 알림을 허용했는지 확인합니다. 방해 금지 모드와 알림 배너 설정은 기기에서 확인해야 합니다.
4. 받는 기기에서 앱을 보고 있을 때와 화면을 잠근 상태를 각각 확인합니다. 발송에 사용한 기기보다 수신 기기의 앱 상태가 중요합니다. `FCM 접수`는 서버가 요청을 받아들인 수이며 시스템 알림 표시나 열람의 확인은 아닙니다.

서비스 워커는 data-only FCM 메시지를 `push` 이벤트에서 직접 표시하고 `event.waitUntil()`로 표시 완료까지 기다립니다. Firebase의 기본 수신 처리는 보이는 앱 창이 있으면 `onMessage`로만 전달하므로 `onBackgroundMessage`에서만 알림을 만들면 앱을 보고 있을 때 시스템 알림이 생기지 않습니다. SDK는 앱 안의 안내와 구독 갱신에 계속 사용합니다. 서버 payload를 notification 메시지로 바꾸거나 별도 백그라운드 알림 표시기를 추가하면 중복 표시될 수 있으므로 변경 시 함께 검증해야 합니다.
...
`workshops/main`은 관리자 전용 원본입니다. `workshops/participants`에는 미발견 보물의 `lat`, `lng`, `outcome`을 제외한 참가자 상태를 저장합니다. 발견 직후에만 해당 좌표가 공개됩니다. 두 문서는 모든 서버 변경에서 같은 트랜잭션으로 갱신됩니다.

클라이언트는 자신의 `members/{uid}` 프로필에서 역할을 확인한 뒤 해당 상태 문서를 구독합니다. 다른 참가자의 인증 프로필과 입장 코드 문서는 읽을 수 없습니다. 지도 방향 안내는 `workshopAction`의 `getGuidance` 액션이며 인증·게임 진행 상태·잠금·GPS 신선도와 정확도를 검증합니다. 지도 안내에서는 정확한 목표 좌표나 꽝 종류를 반환하지 않습니다. 위치 AR용 `getArTarget`은 같은 검증에 더해 근거리·GPS 오차 40m 이하·20초 이내의 위치를 확인한 후 선택한 보물의 좌표만 제공합니다. [AR 구현 범위](ar.md)

이 구조로 처음 전환할 때는 원본 상태를 잠근 트랜잭션에서 참가자 문서를 생성하고, 서버와 화면 배포 후 최종 보안 규칙을 적용합니다. 운영자의 기존 원본 데이터는 삭제하거나 다시 초기화하지 않습니다. 과거 버전의 PWA를 열고 있던 참가자는 새로고침 후 앱 업데이트를 적용해야 합니다.

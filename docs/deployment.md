...
클라이언트는 자신의 `members/{uid}` 프로필에서 역할을 확인한 뒤 해당 상태 문서를 구독합니다. 다른 참가자의 인증 프로필과 입장 코드 문서는 읽을 수 없습니다. 방향 안내는 `workshopAction`의 `getGuidance` 액션이며 인증·게임 진행 상태·잠금·GPS 신선도와 정확도를 검증합니다. 정확한 목표 좌표나 꽝 종류를 반환하지 않습니다.

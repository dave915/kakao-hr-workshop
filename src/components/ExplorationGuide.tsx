...
export default function ExplorationGuide({
  ...
  const cameraOnly = Boolean(current?.cameraOnly);
  ...
  return (
    ...
    >
      ...
      {tracking ? (
        <>
          {cameraOnly ? (
            <div className="guide-camera-search" role="status">
              <Camera size={30} aria-hidden="true" />
              <div>
                <strong>이제 카메라로 찾아보세요</strong>
                <p>가까이 왔어요! 주변을 비추며 보물을 찾아보세요.</p>
              </div>
            </div>
          ) : (
            <>
          ...
          <p className="guide-north">
            {cameraOnly
              ? `보물에서 ${treasure.radius}m 안으로 다가가면 카메라로 획득할 수 있어요.`
              : "위치·방향이 바뀌면 바로 안내해요"}
          </p>
          <div className="guide-actions">
            <button
              className="button dark"
              disabled={disabled || busy || !cameraOnly}
              onClick={onAr}
            >
              <Camera size={16} />
              {busy
